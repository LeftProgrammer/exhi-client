...
const bgImage = resolvePkgUrl('baima-research/top-right-bg.png')
const titleImg = resolvePkgUrl('baima-research/top-right-title.png')
</script>

<template>
  <main class="top-right">
    <img class="top-right__bg" :src="bgImage" alt="" />
    <img class="top-right__title" :src="titleImg" alt="专利技术" />
    <div class="top-right__content">
      <img
        v-if="pointId"
        :src="resolvePkgUrl(`baima-research/${pointId}/${phase}-patent.png`)"
        alt=""
      />
      <div v-else class="top-right__empty">等待选择...</div>
    </div>
  </main>
</template>
